...
export default function OwnerDashboard({ params }: { params: Promise<{ owner: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-bg text-text-primary">
      ...
      {/* ── Sub-header ──────────────────────────────────────────────────────── */}
      <div className="border-b border-border" style={{ background: "rgba(16,25,34,0.92)" }}>
        <div className="max-w-[1440px] mx-auto px-5 h-10 flex items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            ...
            {state.offline ? (
              <span className="pill text-[0.45rem]" style={{ background: "#233648", color: "#92adc9" }}>PENDING</span>
            ) : state.error ? (
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button onClick={handleRefresh} disabled={isRefreshing}
              ...
              {lastUpdated && !isRefreshing && (
                <span className="text-[0.45rem] tabular-nums hidden sm:block" style={{ color: "#4a6a8a" }}>
                  ...
                  <>
                    <ResponsiveContainer width="100%" height={165}>
                      <PieChart>
                        ...
                        <Tooltip contentStyle={{ background: "#192633", border: "1px solid #233648", borderRadius: 8, fontSize: 11 }}
                ...
                {posChartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={Math.max(180, posChartData.length * 30)}>
                    <BarChart layout="vertical" data={posChartData} barSize={11}>
                      ...
                      <XAxis type="number" tick={{ fill: "#4a6a8a", fontSize: 9 }} axisLine={false}
      ...
      <footer className="border-t border-border mt-8 py-4" style={{ background: "rgba(16,25,34,0.6)" }}>
        <div className="max-w-[1440px] mx-auto px-5 flex items-center justify-between">
          <span className="label-xs" style={{ color: "#4a6a8a" }}>{ownerConfig.name} · auto-refresh 30s</span>
          <div className="flex items-center gap-2">
            <div className="w-1.5 h-1.5 rounded-full animate-pulse-neon" style={{ background: c }} />
            <span className="text-[0.5rem] tabular-nums" style={{ color: "#4a6a8a" }}>
    ...
    <div className="card p-5 overflow-hidden">
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "#0a1520" }}>
        <svg viewBox="0 0 1000 500" className="w-full" xmlns="http://www.w3.org/2000/svg">
          {/* Grid */}
          {[0,1,2,3,4].map(i => (
            <line key={`h${i}`} x1="0" y1={i * 125} x2="1000" y2={i * 125} stroke="#1e3347" strokeWidth="0.6" />
          ))}
          {[0,1,2,3,4,5,6].map(i => (
            <line key={`v${i}`} x1={i * 166.7} y1="0" x2={i * 166.7} y2="500" stroke="#1e3347" strokeWidth="0.6" />
          ))}

          {/* Continents */}
          <polygon points="38,72 80,52 212,48 298,76 350,74 360,116 332,124 305,128 298,134 292,148 280,169 270,183 248,214 193,196 155,160 150,114 62,94" fill="#142236" stroke="#233648" strokeWidth="0.8" />
          <polygon points="319,19 444,19 448,80 362,82" fill="#142236" stroke="#233648" strokeWidth="0.8" />
          <polygon points="248,214 285,225 328,219 402,258 405,310 385,360 348,395 315,408 290,390 270,353 268,308 275,265" fill="#142236" stroke="#233648" strokeWidth="0.8" />
          <polygon points="456,165 466,148 466,126 476,108 474,92 490,82 510,72 528,66 542,58 558,52 580,50 602,58 620,76 622,100 618,118 608,133 591,143 573,146 556,145 543,158 528,168 510,168 493,162" fill="#142236" stroke="#233648" strokeWidth="0.8" />
          <polygon points="460,165 493,162 510,168 540,165 573,146 591,143 620,148 630,172 630,213 616,256 600,296 570,345 532,383 500,390 468,366 444,313 436,263 449,213 452,175" fill="#142236" stroke="#233648" strokeWidth="0.8" />
          <polygon points="572,136 603,94 617,61 667,53 750,46 850,50 950,62 994,69 970,95 940,115 920,138 906,152 880,158 858,172 840,196 820,222 790,225 764,218 740,208 718,228 698,228 680,210 660,192 636,182 625,153 617,133 600,150" fill="#142236" stroke="#233648" strokeWidth="0.8" />
          <polygon points="817,272 925,278 919,342 903,356 819,344" fill="#142236" stroke="#233648" strokeWidth="0.8" />
          <polygon points="878,120 892,130 886,150 876,148 878,130" fill="#142236" stroke="#233648" strokeWidth="0.8" />
          <polygon points="968,352 977,360 972,370 965,363" fill="#142236" stroke="#233648" strokeWidth="0.6" />
        ...
        {cities.map(city => {
          ...
          return (
            <div key={city} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[0.55rem]"
              style={{ background: "#101922", border: "1px solid #233648" }}>
...
function BigStat({ label, value, sub, color, loading }: {
  ...
  return (
    <div className="rounded-xl p-4" style={{ background: "#192633", border: "1px solid #233648" }}>
      <div className="label-xs mb-2">{label}</div>
      {loading
        ? <div className="skeleton h-7 w-28 rounded-lg" />
        : <div className="stat-xl tabular-nums animate-number" style={{ color }}>{value}</div>
      }
      {sub && <div className="text-[0.5rem] mt-1" style={{ color: "#4a6a8a" }}>{sub}</div>}
    </div>
  );
}
