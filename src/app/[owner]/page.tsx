...
export default function OwnerDashboard({ params }: { params: Promise<{ owner: string }> }) {
  ...
  // Validate owner
  if (!OWNER_IDS.includes(owner as OwnerId)) {
    notFound();
  }
    ...
    <div className="min-h-screen bg-bg text-text-primary font-sans">
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2.5 shrink-0">
              <div
                ...
                style={{ background: ownerConfig.accentDim }}
              >
                <span className="text-[0.6rem] font-black" style={{ color: ownerConfig.color }}>
                  {ownerConfig.name[0]}
                </span>
              </div>
              <span className="text-text-primary font-bold text-sm tracking-wider">
                {ownerConfig.name.toUpperCase()}
              </span>
              <span className="text-text-muted text-xs">/</span>
              <span
                className="text-[0.6rem] font-semibold tracking-widest"
                style={{ color: ownerConfig.color }}
              >
                {ownerConfig.type} BOT
              </span>

              {/* Status */}
              {state.offline ? (
                <span className="pill bg-text-muted/10 text-text-muted text-[0.5rem]">PENDING</span>
              ) : state.error ? (
                <span className="pill bg-red/10 text-red text-[0.5rem]">ERROR</span>
              ) : state.status ? (
                <div className="flex items-center gap-1">
                  <div
                    className="w-1.5 h-1.5 rounded-full animate-pulse-neon"
                    style={{ backgroundColor: ownerConfig.color }}
                  />
                  <span className="text-[0.5rem] font-semibold" style={{ color: ownerConfig.color }}>LIVE</span>
                </div>
              ) : null}
            </div>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button
                ...
                className={`pill px-3 py-1 text-[0.6rem] font-semibold transition-all ${
                  paused ? "bg-red/15 text-red hover:bg-red/25" : "bg-neon/10 text-neon hover:bg-neon/20"
                }`}
              >
                {paused ? "Resume" : "Pause"}
              </button>
      ...
      <main className="max-w-[1440px] mx-auto px-5 py-5 space-y-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-[0.5rem] text-text-muted uppercase tracking-wider">Cities:</span>
          {ownerConfig.cities.map((city) => (
            <span
              key={city}
              className="text-[0.55rem] px-2 py-1 rounded-lg font-mono"
              style={{ background: ownerConfig.accentDim, color: ownerConfig.color }}
            >
              {city}
            </span>
          ))}
        ...
        {/* Offline/error state */}
        {state.offline && (
          <div className="card p-6 text-center" style={{ borderColor: "rgba(53,77,106,0.5)" }}>
            <div className="text-2xl mb-2">⏳</div>
            <p className="text-text-secondary text-sm font-semibold">{ownerConfig.name}&apos;s bot is not configured yet</p>
            <p className="text-text-muted text-xs mt-1">Set BOT_{ownerId.toUpperCase()}_VPS_URL in environment to enable</p>
          </div>
        )}
